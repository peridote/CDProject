...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(im->m_window))
	{
		...
		if (ImGui::GetIO().MouseClickedPos[0].x > 180.0f && ImGui::GetIO().MouseClickedPos[0].x < 1536.0f)
		{
			if (ImGui::GetIO().MouseClickedPos[0].y > 20 && ImGui::GetIO().MouseClickedPos[0].y < 452)
			{
				ImVec2 diff_left = ImGui::GetMouseDragDelta(0, 0.0f);
		...
		if (ImGui::GetIO().MousePos.x > 180.0f && ImGui::GetIO().MousePos.x < 1536.0f)
		{
			if (ImGui::GetIO().MousePos.y > 20 && ImGui::GetIO().MousePos.y < 452)
			{
				float wheelcounter = ImGui::GetIO().MouseWheel;
				trans_z += -(wheelcounter) / static_cast<Real>(1.0);
			}
		}
		...
		if (ImGui::GetIO().MouseClickedPos[1].x > 180.0f && ImGui::GetIO().MouseClickedPos[1].x < 1536.0f)
		{
			if (ImGui::GetIO().MouseClickedPos[1].y > 20 && ImGui::GetIO().MouseClickedPos[1].y < 452)
			{
				ImVec2 diff_right = ImGui::GetMouseDragDelta(1, 0.0f);
